...
class DosPlotter(object):
	...
	def get_ax(self, ax, xlim=None, ylim=None, plot_both_spin=True,
					x_tick_space=10.0, y_tick_space=1.0, fontsize=20):
		"""
		Get a matplotlib ax for the DOS. Should give an initialized ax object as the first input.

		Args:
			ax: initialized ax using subplot
			xlim: Specifies the x-axis limits. Set to None for automatic
				determination.
			ylim: Specifies the y-axis limits.
			plot_both_spin=False, to add up up-spin and down-spin(for non-magnetic materials)
		"""
		...
		alldensities = []
		allenergies = []
		#plt = get_publication_quality_plot(dimension_x, dimension_y) #dimension
		...
		for i, key in enumerate(keys):
			...
			for spin in [Spin.up, Spin.down]:
				if spin in alldensities[i]:
					...
					if spin == Spin.down:
						energies.reverse()
						densities.reverse()
					y.extend(energies)  #YC:change x and y to flip the DOS plot
					x.extend(densities)
			...
			#             label=str(key))
			if plot_both_spin:
				ax.plot(x, y, color=colors[i % ncolors],
						 label=str(key), linewidth=3)
			else:
				ax.plot(map(lambda a:a*2,x), y, color=colors[i % ncolors],
						 label=str(key), linewidth=3)   #YC_DOS*2 to account for spin
				print('Reminder: this is for spin_up + spin_down, please remove the negative DOS by setting xlim')

			if not self.zero_at_efermi:
				ylim = ax.get_ylim()
				ax.plot([self._doses[key]['efermi'],
		...
		ax.set_xlabel('DOS (states/eV/cell)', fontsize=fontsize)
		from numpy import arange
		from matplotlib.ticker import FormatStrFormatter
		ax.set_xticks(arange(xlim[0], xlim[1]+0.1, x_tick_space))
		ax.set_xticklabels(arange(xlim[0], xlim[1]+0.1, x_tick_space), fontsize=fontsize)
		ax.set_yticks(arange(ylim[0], ylim[1]+0.1, y_tick_space))
		ax.set_yticklabels(arange(ylim[0], ylim[1]+0.1, y_tick_space), fontsize=fontsize)
	...
	def get_plot(self, xlim=None, ylim=None, plot_both_spin=True, dimension_x=3, dimension_y=6, 
					x_tick_space=10.0, y_tick_space=1.0, fontsize=20):
		"""
		...
		for i, key in enumerate(keys):
			...
			if plot_both_spin:
				...
				ppl.plot(map(lambda a:a*2,x), y, color=colors[i % ncolors],
						 label=str(key), linewidth=3)   #YC_DOS*2 to account for spin
				print('Reminder: this is for spin_up + spin_down, please remove the negative DOS by setting xlim')
		...
		#plt.ylabel('Energy (eV)')
		plt.xlabel('DOS (states/eV/cell)', fontsize=fontsize)
		from numpy import arange
		plt.xticks(arange(xlim[0], xlim[1]+0.1, x_tick_space), fontsize=fontsize)
		plt.yticks(arange(ylim[0], ylim[1]+0.1, y_tick_space), fontsize=fontsize)
...
class BSPlotter(object):
	...
	def _maketicks_ax(self, ax, fontsize=20):
		"""
		...
		ax.set_xticklabels(uniq_l, fontsize=fontsize)
	...
	def get_ax(self, ax, zero_to_efermi=True, ylim=None, smooth=False,
				 vbm_cbm_marker=False, smooth_tol=None, plot_both_spin=True, y_tick_space=0.5, fontsize=20):
		"""
		Get a matplotlib ax object for the bandstructure plot.
		Blue lines are up spin, red lines are down
		spin.

		Args:
			ax: should provide an ax object as input
			...
			ylim: Specify the y-axis (energy) limits; by default None let
				...
				efermi-10 and efermi+10 if metal i.e [-5, 3]
		...
		if not smooth:
			for d in range(len(data['distances'])):
				for i in range(self._nb_bands):
					...
					if self._bs.is_spin_polarized and plot_both_spin:
						ax.plot(data['distances'][d],
...
#                                 linewidth=band_linewidth)
		...
		self._maketicks_ax(ax, fontsize=fontsize)
		from numpy import arange
		ax.set_yticks(arange(ylim[0], ylim[1]+0.1, y_tick_space))
		ax.set_yticklabels(arange(ylim[0], ylim[1]+0.1, y_tick_space), fontsize=fontsize)
		...
		if not zero_to_efermi:
			ef = self._bs.efermi
			ax.axhline(ef, linewidth=2, color='k', linestyle='--')
		else:
		...
		if ylim is None:
			if self._bs.is_metal():
				...
				if vbm_cbm_marker:
					for cbm in data['cbm']:
						ax.scatter(cbm[0], cbm[1], color='r', marker='o',
									s=100)
					for vbm in data['vbm']:
						ax.scatter(vbm[0], vbm[1], color='g', marker='o',
									s=100)	
				ax.set_ylim(data['vbm'][0][1] + e_min,
						 data['cbm'][0][1] + e_max)
		else:
			ax.set_ylim(ylim)
	...
	def get_plot(self, zero_to_efermi=True, ylim=None, smooth=False,
				 vbm_cbm_marker=False,smooth_tol=None, plot_both_spin=True, dimension_x=12, dimension_y=6, fontsize=20):
		"""
...
#                                 linewidth=band_linewidth)
		...
		if not zero_to_efermi:
			ef = self._bs.efermi
			plt.axhline(ef, linewidth=2, color='k', linestyle='--')
		else:
