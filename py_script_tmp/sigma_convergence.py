...
topval=1  # top of the valence band (hack, was 20, but need this to work)
    ...
    while True:
        ...
        if line.find('k =')!=-1 and line.find('spin =')!=-1:
            ...
            if abs(kx)<1E-10 and abs(ky)<1E-10 and abs(kz-0.5)<1E-10:
                Xindex=int(kindex)
            if abs(kx-0.5)<1E-10 and abs(ky-0.5)<1E-10 and abs(kz-0.5)<1E-10:
                Rindex=int(kindex)
...
for number in range(len(xarray)-1,-1,-1):
    ...
    dirRconv=globaldata[0][Rindex][topval]-globaldata[0][Rindex][topval-1]
    dirRcur=globaldata[number][Rindex][topval]-globaldata[number][Rindex][topval-1]
    dirRerror.append(abs(dirRconv-dirRcur)*1000)
...
plot(xarray,dirRerror,'m-^',label=r'$R-R$')

ylabel('Energy error (meV)')
xlabel(r'# screended cutoff (Ry)')
