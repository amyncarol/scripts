...
class BoltztrapAnalyzer(object):
    ...
    ##same name for two different variable, too bad; change the boolean one 
    ## to is_doping_levels
    def get_seebeck(self, output='eigs', is_doping_levels=True):
        """
            Gives the seebeck coefficient (microV/K) in either a
            full 3x3 tensor form, as 3 eigenvalues, or as the average value
            (trace/3.0) If is_doping_levels=True, the results are given at
            ...
            Args:
                ...
                is_doping_levels (boolean): True for the results to be given at
                different doping levels, False for results
                at different electron chemical potentials

            Returns:
                If is_doping_levels=True, a dictionary {temp:{'p':[],'n':[]}}.
        ...
        return BoltztrapAnalyzer._format_to_output(self._seebeck,
                                                   ...
                                                   output,
                                                   is_doping_levels, 1e6)

    def get_conductivity(self, output='eigs', is_doping_levels=True,
    ...
    def get_power_factor(self, output='eigs', is_doping_levels=True,
                         relaxation_time=1e-14):
        """
        ...
        is_doping_levels=True, the results are given at
        ...
        Returns:
            If is_doping_levels=True, a dictionnary {temp:{'p':[],'n':[]}}. The
        ...
        return BoltztrapAnalyzer._format_to_output(result, result_doping,
                                                   output, is_doping_levels,
                                                   multi=1e6 * relaxation_time)

    def get_thermal_conductivity(self, output='eigs', is_doping_levels=True,
                                 relaxation_time=1e-14):
        """
        ...
        Returns:
            If is_doping_levels=True, a dictionary {temp:{'p':[],'n':[]}}. The
    ...
    def get_zt(self, output='eigs', is_doping_levels=True, relaxation_time=1e-14,
               kl=1.0):
        """
    ...
    def get_average_eff_mass_vbm_cbm(self, output='eigs'):
        """
        Give effective mass in either a full 3x3 tensor
            form, as 3 eigenvalues, or as the average value
            (trace/3.0), it is given as a dict {'p':{temp:mass_p},'n':{temp:mass_n}}
            ...
            Returns:
                returns a {'p':{temp:mass_p},'n':{temp:mass_n}} dictionary. The result contains either
                the sorted three eigenvalues of the symmetric
                effective mass tensor (format='eigs') or a full tensor (3x3
                array) (output='tensor') or as an average
                (output='average').
        """     
        result = {doping_type: {temp: None for temp in self._cond} for
                         doping_type in self.doping}

        mass = self.get_average_eff_mass_mu(output=output)
        mu = self.mu_steps
        vbm_index=len(list(filter(lambda x:x<0,mu)))
        cbm_index=len(list(filter(lambda x:x<self.gap,mu)))
        #print(mu[vbm_index], mu[cbm_index])
        for temp in self._cond:
            result[u'p'][temp]=mass[temp][vbm_index]
            result[u'n'][temp]=mass[temp][cbm_index]
        return result


    def get_average_eff_mass_mu(self, output='eigs'):
        """
        Give effective mass in either a full 3x3 tensor
            form, as 3 eigenvalues, or as the average value
            (trace/3.0), it is given as a series
            of electron chemical potential values
        ...
        effective_mass = {temp: [] for temp in self._cond}
        concentration = {temp: [1e24 * i / self.vol for i in self._carrier_conc[temp]]
                for temp in self._carrier_conc}
        for temp in self._cond:
            for i in range(len(self._cond[temp])):
                if output == 'tensor':
                    try:
                        effective_mass[temp].append(np.linalg.inv(
                            np.array(self._cond[temp][i])) \
                                                           *
                                                           concentration[temp][
                                                             i] * 10 ** 6 *
                                                           e ** 2 /
                        ...
                        effective_mass[temp].append(np.zeros((3,3)))

                elif output in ['eig', 'eigs']:
                    try:
                        effective_mass[temp].append(
                        ...
                        #print("LinAlgError: Singular matrix")
                        effective_mass[temp].append(np.zeros(3))
                    ...
                    try:
                        ...
                        #print("LinAlgError: Singular matrix")
                        effective_mass[temp].append(0.0)
    ...
    def get_extreme(self, target_prop, maximize=True, min_temp=None,
        ...
        if target_prop.lower() == "seebeck":
            d = self.get_seebeck(output="eigs", is_doping_levels=True)

        elif target_prop.lower() == "power factor":
            d = self.get_power_factor(output="eigs", is_doping_levels=True)

        elif target_prop.lower() == "conductivity":
            d = self.get_conductivity(output="eigs", is_doping_levels=True)

        elif target_prop.lower() == "kappa":
            d = self.get_thermal_conductivity(output="eigs",
                                              is_doping_levels=True)
        elif target_prop.lower() == "zt":
            d = self.get_zt(output="eigs", is_doping_levels=True)
    ...
    @staticmethod
    def _format_to_output(tensor, tensor_doping, output, is_doping_levels,
                          multi=1.0):
        if is_doping_levels:
    ...
    @staticmethod
    def parse_cond_and_hall(path_dir, doping_levels):
        """
    ...
    @staticmethod
    def from_files(path_dir, doping_levels=None, dos_spin=1):
        """
        get a BoltztrapAnalyzer object from a set of files

        Args:
            path_dir: directory where the boltztrap files are
        doping_levels: [float] in cm^-3 #YC_this is an ugly trick, but it works!
        ...
        """
        ##parse_outputtrans() cannot get doping_levels
        run_type, warning, efermi, gap, bad_doping_levels = \
