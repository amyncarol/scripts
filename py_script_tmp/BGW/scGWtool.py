...
#                                                                                         #
# File needed: sigma_hp_col.log sigma_hp_row wfn_old.h5                                   #
...
def read_sigma_vxc(ln, nb, sigma, vxc, bmap):
    r"""
    Called by rea_sigma_hp, read vxc and sigma matrices
    """
                ...
                for ib1 in range(nb):
                    for ib2 in range(nb):
                        ...
                        if int(spb[0]) != bmap[ib1] or int(spb[1]) != bmap[ib2] or spb[3] != "real":
                            print "Info not matching in off-diag reading in sigmg_hp_col.log"
                            sys.exit(0)
                        sigma[ik][ib1][ib2] += float(spb[7])  # Initialization MUST be ZEROS
                        vxc[ik][ib1][ib2] += float(spb[8])   # Initialization MUST be ZEROS
...
def diag(mat, deg = False, herm = False):
    r"""
...
def wfn_update(hammf, hamqp, bmap, kmap, evecs):
    ...
    for ik in range(nk):
        print
    	print "Doing", ik, "out of", nk
    	print "This is the kpoint", kmap[ik], "in wavefunction"
        print "Python kidx is", kmap[ik]-1
        ...
        for ib in range(bandmin-1, bandmax):   # python starts from 0
    		wfncoef_ik[ib-(bandmin-1), : ] = f_in['wfns/coeffs'][ib, 0, numabv : numabv+ngk[kmap[ik]-1], 0] \
						 + 1.0j * f_in['wfns/coeffs'][ib, 0, numabv : numabv+ngk[kmap[ik]-1], 1]

    	for ib1 in range(nb):
    		for ib2 in range(nb):
    			wfn_rotate[ib1, :] += evecs[ik][ib2][ib1] * wfncoef_ik[ib2, :]  # Pay attention to evec index order

    	for ib in range(bandmin-1, bandmax):
    		f_out['wfns/coeffs'][ib, 0, numabv : numabv+ngk[kmap[ik]-1], 0]  =  wfn_rotate[ib-(bandmin-1), :].real
    		f_out['wfns/coeffs'][ib, 0, numabv : numabv+ngk[kmap[ik]-1], 1]  =  wfn_rotate[ib-(bandmin-1), :].imag
    ...
    for ik in range(nk):
        ...
        for ib in range(nbtot):
            bandmf[ik][ib] = f_in['mf_header/kpoints/el'][ispin, kmap[ik]-1, ib] * ryd

        bandqp[ik, :] = extrap(hammf[ik], hamqp[ik], bmap, bandmf[ik])[:]
        
        for ib in range(nbtot):
            f_out['mf_header/kpoints/el'][ispin, kmap[ik]-1, ib] = bandqp[ik][ib] / ryd
...
def write_eqp_vxc(bandqp, hamqp, sigma, bmap, kpts):
    ...
    for ik in range(nk):
        ...
        for ib in range(nb):
            ln = "        1    " + str(bmap[ib]) + "   " \
                    + str(vxc[ik][ib][ib].real) + "   " + str(vxc[ik][ib][ib].imag) + "\n"
            fvxc.write(ln)

        for ib1 in range(nb):
            for ib2 in range(nb):
                ln = "        1    " + str(bmap[ib1]) + "   "  + str(bmap[ib2]) + "   " \
                        + str(vxc[ik][ib1][ib2].real) + "   " + str(vxc[ik][ib1][ib2].imag) + "\n"
                fvxc.write(ln)
